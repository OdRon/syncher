...
class Random
{
	...
	public static function get_current_sex_query($param, $sex)
	{
    	$sql = 'SELECT facility_id, count(*) as totals ';
		$sql .= 'FROM ';
	...
	public static function save_sex_results()
	{
		ini_set("memory_limit", "-1");
        config(['excel.import.heading' => true]);
		$path = public_path('facilities.csv');
		...
		$sql = self::get_current_sex_query(1, 1);
		$male_one = collect(DB::select($sql));

		$sql = self::get_current_sex_query(2, 1);
		$male_two = collect(DB::select($sql));

		$sql = self::get_current_sex_query(4, 1);
		$male_four = collect(DB::select($sql));

		$sql = self::get_current_sex_query(1, 1);
		$female_one = collect(DB::select($sql));

		$sql = self::get_current_sex_query(2, 1);
		$female_two = collect(DB::select($sql));

		$sql = self::get_current_sex_query(4, 1);
		$female_four = collect(DB::select($sql));
		...
		$file = '2018_sex_totals_by_most_recent_test';
